...
class RegistrationViewPresenter {
    ...
    private func signUp(_ username: String?, _ mobileNumber: String?, _ email: String?, _ password: String?) {
        ...
        AuthService.shared.isEmailFree(email!) { [weak self] result in
            ...
            case .failure(let error):
                self.viewController?.showAlertController(alertController: AuthHelper.shared.setupAlertController(title: "Fail",
    ...
    private func isValidSingUp(_ username: String?, _ mobileNumber: String?, _ email: String?, _ password: String?) -> Bool {
        guard AuthHelper.shared.isValidSingUp(username, mobileNumber, email, password) else {
            viewController?.showAlertController(alertController: AuthHelper.shared.setupAlertController(title: "Failed registration",
    ...
    private func isValidPassword(_ password: String) -> Bool {
        guard AuthHelper.shared.isValidPassword(password) else {
            viewController?.showAlertController(alertController: AuthHelper.shared.setupAlertController(title: "Invalid password",
    ...
    private func setupAlertForUsesEmail() -> UIAlertController {
        ...
        let alertController = AlertControllerMaker.makeAlertWithCancel(alertTitle, alertMessage, style: .alert, okTitle, cancelTitle) { action in
    ...
    private func verifMobileNumber(_ mobileNumber: String) {
        AuthHelper.shared.verifMobileNumber(mobileNumber: mobileNumber) { [weak self] result in
            ...
            case false:
                ...
                self?.viewController?.showAlertController(alertController: AuthHelper.shared.setupAlertController(title: title,
                                                                                                                  message: message,
                                                                                                                  handler: nil))
            }
        }
